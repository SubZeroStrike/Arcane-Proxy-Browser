...
function decryptUrl(encrypted, sessionId) {
  ...
  try {
    return xorCrypt(
      Buffer.from(decodeURIComponent(encrypted), "base64"),
      key
    ).toString("utf8");
  } catch {
    return null;
...
(function(){
  ...
  window.WebSocket = function(url, protocols) {
    try {
      var f = url.startsWith('wss://')||url.startsWith('ws://') ? url : null;
      if (f) {
        var loc = window.location;
        var wsProxyUrl = (loc.protocol==='https:'?'wss:':'ws:')+'//'+loc.host+'/api/proxy/ws?sid='+_sid+'&raw='+encodeURIComponent(f);
        return protocols ? new _WS(wsProxyUrl, protocols) : new _WS(wsProxyUrl);
      }
    } catch(e) {}
  ...
  result = result.replace(
    /(href|src|action|data-src|data-lazy|poster|data-original)=(['"])(.*?)\2/gi,
    (m, attr, q, val) => {
      val = val.trim();
      if (!val) return m;
...
app.get("/api/proxy/asset", async (req, res) => {
  const url = req.query.raw
    ? req.query.raw
    : decryptUrl(req.query.url, req.sessionId);
  ...
  try {
    ...
    if (ct.includes("text/html")) {
      ...
      return res.send(
        rewriteUrls(upstream.body.toString("utf8"), url, req.sessionId)
      );
    }
    if (ct.includes("text/css")) {
      ...
      css = css.replace(/url\(\s*(['"]?)([^'"\)]+)\1\s*\)/gi, (m, q, val) => {
        try {
          ...
          if (!full) return m;
          return `url("/api/proxy/asset?sid=${
            req.sessionId
          }&raw=${encodeURIComponent(full)}")`;
        } catch {
          return m;
        }
      });
...
async function handlePage(req, res) {
  let url = req.query.raw
    ? req.query.raw
    : decryptUrl(req.query.url, req.sessionId);
  if (!url) return res.status(400).send("Bad URL");
  // Try to fix malformed URLs before giving up
  try {
    new URL(url);
  } catch {
    try {
      ...
      console.error("[page] Invalid URL:", JSON.stringify(url));
      return res.status(400).send("Invalid URL: " + url.slice(0, 100));
    }
  }
...
// ── Legacy routes ─────────────────────────────────────────────────────────────
app.get("/api/proxy/primary", (req, res) =>
  res.redirect(307, `/api/proxy/page?sid=${req.sessionId}&url=${req.query.url}`)
);
app.get("/api/proxy/secondary", (req, res) =>
  res.redirect(307, `/api/proxy/page?sid=${req.sessionId}&url=${req.query.url}`)
);
...
app.get("/health", (req, res) => {
  ...
  res.json({
    status: "ok",
    sessionKey: SESSION_KEYS.get(sid).toString("base64"),
  });
});
...
server.on("upgrade", (req, socket, head) => {
  ...
  const rawUrl = new URL(req.url, "http://localhost").searchParams.get("raw");
  if (!rawUrl) return socket.destroy();
